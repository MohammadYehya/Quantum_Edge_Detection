...
images = Image.open('./test16.png')
images = image_preparation(images)
# images = make_square_image(images)
fig = plt.figure()
fig.add_subplot(2,1,1)
plt.imshow(images, cmap='gray')

qc = make_circ(images, numQubits)
# qc.draw(output = 'mpl')
counts = get_counts(qc, numQubits)
retrieve_image = decoding(counts, numQubits)
